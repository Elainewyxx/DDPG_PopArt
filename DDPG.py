...
parser.add_argument("--env_name", default="Pendulum-v0")   # VibrationEnv  Pendulum
...
parser.add_argument('-PopArt_is', '--PopArt_is', default=True, type=bool, help="use PopArt")
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.agent_PopArt = PopArt('POPART', self.critic, state_dim + action_dim, 1, 1, lr, beta)
    ...
    def update(self):
        ...
        for it in range(args.update_iteration):
            ...
            # Q_loss , self.critic = self.agent_PopArt.forward(state, action,  target_Q)
            Q_loss , _ = self.agent_PopArt.forward(state, action,  target_Q)
            # print('Q_loss.: {}'.format(Q_loss))
            # print(self.critic.l3.weight)
            # print(self.critic.l3.bias)

            self.writer.add_scalar('Loss/Q_loss', Q_loss, global_step=self.num_critic_update_iteration)
            # critic_loss = Q_loss
            # input()
            '''
            ...
            # actor_loss = -self.agent_PopArt.output(state, self.actor(state)).mean()
            ...
            for t in count():
                ...
                if done or t >= args.max_length_of_trajectory:
                    ...
                    if args.env_name == 'VibrationEnv-v0':
                        ...
                        agent.writer.add_scalar('Action/input', info['input'], global_step=i)
